...
class BiCrossAttention(nn.Module):
    def __init__(self, input_dim=768, hidden_dim=384, num_heads=4, batch_first=True):
    ...
    def forward(self, featureA, featureB):
        ...
        attn_AB, _ = self.cross_attn_AB(query=hA, key=hB, value=hB)
        attn_BA, _ = self.cross_attn_BA(query=hB, key=hA, value=hA)
        attn_cat = torch.cat([attn_AB, attn_BA], dim=-1)
        out = self.layer_after(attn_cat)
        return out
...
class ExpressionIntesityPrediction(nn.Module):
    def __init__(self, config):
        super(ExpressionIntesityPrediction, self).__init__()
        ...
        self.regressor = Regressor(config, 768, 1)
