...
class ExpressionIntesityClassification(nn.Module):
    def __init__(self, config):
        super(ExpressionIntesityClassification, self).__init__()
        ...
        if config['num_class'] == 3:
            self.classification = Classification(config, 768, 3)
        elif config['num_class'] == 4:
            self.classification = Classification(config, 768, 4)
        elif config['num_class'] == 2:
            self.classification = Classification(config, 768, 1)
    ...
    def forward(self, X, mask):
        ...
        preds = self.classification(output)
