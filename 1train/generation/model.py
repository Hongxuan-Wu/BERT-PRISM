...
class PromoterGenerationDiffusion(nn.Module):
    ...
    def forward(self, X, mask, word_freq_logits, mode):
        if mode == 'train':
            # During training, time step t is randomly selected
            metrics = diffusion_word_freq.compute_kl_reverse_process(
                x_start=X,
                t=self.diffusion_instance.sample_t().to(X.device),
                denoise_fn=self.denoise_fn,
                diffusion=self.diffusion_instance,
                target_mask=mask,
                hybrid_lambda=self.config['hybrid_lambda'],
                predict_x0=self.config['predict_x0'],
                word_freq_logits=word_freq_logits
            )
            return metrics
        elif mode == 'valid':
            # During verification, increase eval_step_size for quick verification
